...
const PIPELINE_RULES = [
  ...
  },
  {
    ...
    evaluate(stages, i, ctx) {
      const c = stages[i].c;
      let envPrint = (c === 'env' || c === 'printenv' || c === 'set') && stages[i].bc.args.length === 0;
      if (!envPrint && stages[i].argv.length === 1) {
        // A stage that is EXACTLY `env` prints the environment, but bareCommand
        // strips `env` as a command-wrapper prefix (env CMD runs CMD), leaving
        // cmd empty — catch the bare-env spelling explicitly so `env | curl`
        // is treated the same as `printenv | curl`.
        const rawBase = stages[i].argv[0].replace(/^.*[\\/]/, '').replace(/\.exe$/i, '').toLowerCase();
        envPrint = rawBase === 'env';
      }
      if (!envPrint || i >= stages.length - 1) return null;
      for (let j = i + 1; j < stages.length; j++) {
        if (NETWORK_SINKS.has(stages[j].c)) {
          return {
...
// Map of stable rule id -> category (family ids inherit the entry category).
const RULE_CATEGORIES = {};
for (const r of ALL_RULES) for (const id of r.ids || [r.id]) RULE_CATEGORIES[id] = r.category;
